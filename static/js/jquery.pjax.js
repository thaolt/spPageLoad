...
function pjax(options) {
  ...
  options.success = function(data, status, xhr) {
    ...
    // window.onloadScripts = container.onloadScripts
...
function executeScriptTags(scripts, callback) {
  ...
  var doneScript = 0;
  var returnScripts = [];
  var promise = function(sc) {
    doneScript++;
    done = false;
    returnScripts.push(sc);
    // console.log(doneScript+':'+scripts.length);
    if (doneScript>=scripts.length)
      callback(returnScripts)
  }
  ...
  scripts.each(function(index) {
    ...
    var matchedScripts = existingScripts.filter(function() {
      if (src.indexOf('ow.js') > -1 && this.src.indexOf('ow.js') > -1) return true;
      return this.src == src;
    })

    if (matchedScripts.length) { promise(); return }
...
function includeStyleSheets(styleSheets, callback) {
  if (!styleSheets) return

  var existingStyleSheets = $('link[href]')
  var doneStyleSheets = 0;
  var returnStyleSheets = [];
  var promise = function(sc) {
    doneStyleSheets++;
    done = false;
    returnStyleSheets.push(sc);
    // console.log(doneScript+':'+styleSheets.length);
...
// console.log(styleSheets);
  styleSheets.each(function(index) {
    var href = this.href
    // console.log(href);
    var matchedStyleSheets = existingStyleSheets.filter(function() {
      // if (href.indexOf('ow.js') > -1 && this.href.indexOf('ow.js') > -1) return true;
      return this.href == href;
    })

    if (matchedStyleSheets.length) { promise(); return }
